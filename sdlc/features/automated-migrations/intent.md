...
- **Mechanism:** Supabase CLI `db push` against the tracked history, vs. running
  the `.sql` files directly (e.g. psql) — which is the canonical tool? (Issue #88
...
- **Where the gate lives:** a GitHub Environments "required reviewers" approval,
  a manual `workflow_dispatch`, or a label — and whether prod always requires
  approval regardless of additive/destructive.
