...
## `core-geo`: a published entity pack, re-runnable but frozen
...
`core-geo` is the sole owner of every shared geographic entity — continents, sovereign countries, capital cities, and core cities (see [../knowledge-graph/identity.md](../knowledge-graph/identity.md)). It ships `entities.jsonl` only: no statements, no generators, so it yields no questions on its own. It exists so that the topic packs — `core-cities` and later `capital-cities`, `continental-countries` — can ship statements over shared identity and no entities of their own, which is what the single-ownership assembly requires.
