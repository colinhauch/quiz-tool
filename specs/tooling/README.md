...
The import pipeline is deferred, but its *requirements* are not. One decision in the shipped design exists only to serve it:
...
Intended sources: Wikidata (facts, labels, aliases in every language), GeoNames (city lists by population), REST Countries, Wikimedia Commons (license-tagged images).
