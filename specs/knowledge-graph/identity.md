...
There is a second mechanism, and it may be the better one: **state the disagreement as facts.** Rather than ranking two capital claims, assert both, and where the disagreement is itself the interesting thing, say so directly — `s(Q_government_of_israel, claims, Q1218)` and `s(Q_state_of_palestine, claims, Q1218)`. The claimant is a *government*, not a country; that precision is what keeps the fact stateable. This needs no engine support, no flag, and no editorial layer; it is the uniform fact model doing its job. See [../tooling/mvp-bootstrap.md](../tooling/mvp-bootstrap.md), where the MVP defers such entities to a later `disputed-land` pack on exactly this basis.
