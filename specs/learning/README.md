...
The rule has since paid out in a way worth recording, because it is the kind of evidence that is easy to lose. The answer log stores a card id and nothing about where that card came from. When the My Answers summary wanted a per-pack breakdown (#239), the owning pack was derived at read time from the card id — the same way the rendered question text and the accepted answer already were — and every answer ever logged named its pack the moment the derivation shipped. No column, no backfill, no migration, and no window of history that reads differently from the rest. Storing the owner at write time would have bought nothing except a migration and a permanent seam between answers logged before and after it.

The price is the one this rule always charges: a card whose pack has changed no longer resolves, so it has no owner to report. That is treated as a category rather than hidden — those answers are counted and labelled as their own group, since dropping them would make every other share wrong.
