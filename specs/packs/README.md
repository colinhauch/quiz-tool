...
**A pack can also be withheld by the operator, not the learner.** `packs/catalog.json` (a sparse `packId → policy` map, read at boot) governs which packs are *offered at all* — `hidden` drops a pack from the picker even though it yields questions, and a reserved `tier` will one day gate on a learner's entitlement. This is a different axis from selection: the catalog decides the menu, the learner selects from it. It stays out of `pack.json` because visibility and price are product policy on a different lifecycle from a pack's authoring facts, and premium is inherently per-*user* — which a pack cannot know. It is applied at the same `/packs` boundary as selection, and like selection it never unloads anything: a hidden pack stays in the graph so its recorded cards still resolve. The first use is retiring `core-cities` (a bare city has no assertion worth quizzing; `capital-cities` replaces it) by hiding rather than deleting it. See `docs/adr/0003-pack-visibility-lives-in-a-catalog.md`.
